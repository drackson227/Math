/* ============================================================
   Application principale — Maths 4GT
...
function loadSavedData() {
  try {
    const saved = localStorage.getItem('maths4gt_data');
...
function saveData(data) {
  try {
    localStorage.setItem('maths4gt_data', JSON.stringify(data));
...
function exportProgress() {
  ...
  a.href = url;
  a.download = `maths4gt-progres-${stamp}.json`;
...
function updateLevel(data) {
  if (data.xp >= 300) {
    data.level = 'Expert 4GT';
    if (!data.badges.includes('Analyste')) data.badges.push('Analyste');
    if (!data.badges.includes('Expert 4GT')) data.badges.push('Expert 4GT');
  } else if (data.xp >= 100) {
    data.level = 'Analyste';
    if (!data.badges.includes('Analyste')) data.badges.push('Analyste');
  } else {
...
function setTheme(theme) {
  ...
  localStorage.setItem('maths4gt_theme', theme);
...
function setTextSize(size) {
  ...
  localStorage.setItem('maths4gt_textsize', size);
...
function setStageScene(stage, html, animate, opts) {
  ...
  Object.keys(oldRects).forEach(tid => {
    ...
    if (partner && oldRects[partner]) {
      ...
      const a = center(r);
      const iAmAdd = oldEls[tid].classList && oldEls[tid].classList.contains('tadd');
      const partnerAdd = oldEls[partner] && oldEls[partner].classList && oldEls[partner].classList.contains('tadd');
      // point de rencontre = position du terme ORIGINAL (non rouge) ; le rouge vient l'y rejoindre
      const meet = (iAmAdd && !partnerAdd) ? center(oldRects[partner])
                 : (!iAmAdd && partnerAdd) ? a
                 : { x: (a.x + center(oldRects[partner]).x) / 2, y: (a.y + center(oldRects[partner]).y) / 2 };
      // 1) glisse jusqu'au point de rencontre (le rouge « tombe sur » l'autre)
      gsap.to(c, { x: meet.x - a.x, y: meet.y - a.y, duration: 0.5, ease: 'power2.inOut' });
      // 2) une fois superposés : petit choc puis disparition
      gsap.to(c, { scale: 0.15, opacity: 0, duration: 0.3, delay: 0.5, ease: 'power2.in', onComplete: () => c.remove() });
      // le « 0 » + l'onde de choc ne sont déclenchés qu'une fois par paire
      if (tid < partner) {
        const ghost = { getBoundingClientRect: () => ({ left: meet.x - 6, top: meet.y - 6, width: 12, height: 12 }) };
        burstAt(ghost, stage, fxLayer, 500);
        const zero = document.createElement('span');
        zero.className = 'atok atok-result';
        zero.textContent = '0';
        zero.style.cssText = 'position:absolute; left:' + (meet.x - stageRect.left) + 'px; top:' + (meet.y - stageRect.top) + 'px; transform:translate(-50%,-50%);';
        fxLayer.appendChild(zero);
        gsap.fromTo(zero, { opacity: 0, scale: 0.2 },
          { opacity: 1, scale: 1.35, duration: 0.42, delay: 0.5, ease: 'back.out(2.2)',
            onComplete: () => gsap.to(zero, { opacity: 0, y: -12, duration: 0.5, delay: 0.5, onComplete: () => zero.remove() }) });
      }
  ...
  eq.querySelectorAll('[data-tid]').forEach(t => {
    ...
    if (oldRects[tid]) {
      ...
      if (src && oldRects[src]) {
        ...
        if (isMergeTo) burstAt(t, stage, fxLayer, delay * 1000);
      }
    }
  });
}
...
(function initScratchpad() {
  function safeCalc(raw) {
    ...
    let e = raw.replace(/√/g, 'Math.sqrt').replace(/π/g, 'Math.PI').replace(/\^/g, '**').replace(/×/g, '*').replace(/÷/g, '/').replace(/,/g, '.');
  ...
  function build() {
    ...
    panel.innerHTML =
      ...
      '<div style="display:flex; flex-wrap:wrap; gap:5px; margin-bottom:8px;">' +
        ['7','8','9','/','4','5','6','*','1','2','3','-','0','.','(',')','√(',')','^','π','C','='].map(function(k){
          return '<button type="button" class="scratch-key" data-k="'+k+'" style="flex:1 0 17%; padding:7px 0; border-radius:8px; border:1px solid var(--border-subtle); background:var(--bg-main); color:var(--text-primary); cursor:pointer; font-size:13px;">'+k+'</button>';
        }).join('') +
      '</div>' +
    ...
    panel.querySelectorAll('.scratch-key').forEach(k => k.addEventListener('click', () => {
      const v = k.dataset.k;
      if (v === 'C') { calc.value = ''; recalc(); calc.focus(); return; }
      if (v === '=') { recalc(); return; }
      calc.value += v; recalc(); calc.focus();
    }));
    // restaure le brouillon entre sessions
    const pad = panel.querySelector('#scratch-pad');
    try { pad.value = localStorage.getItem('maths4gt_scratch') || ''; } catch (_) {}
    pad.addEventListener('input', () => { try { localStorage.setItem('maths4gt_scratch', pad.value); } catch (_) {} });
  }
...
function toggleFlashcardEffect(effect) {
  ...
  localStorage.setItem('maths4gt_flashcard_fx', activeFlashcardEffects.join(','));
}
...
function hidePomodoroOverlay() {
  ...
  localStorage.setItem('maths4gt_pomodoro_running', 'true');
}
...
function setAnimLevel(level) {
  ...
  localStorage.setItem('maths4gt_anim', level);
...
function setPomodoroDisplay(mode) {
  ...
  localStorage.setItem('maths4gt_pomo_display', mode);
...
function setBgEffect(effect) {
  ...
  localStorage.setItem('maths4gt_bg', effect);
...
let ambientVolumePct = (() => {
  const v = parseInt(localStorage.getItem('maths4gt_volume'), 10);
...
function setAmbientVolume(pct) {
  ...
  localStorage.setItem('maths4gt_volume', ambientVolumePct);
...
function setAmbientMusic(type) {
  ...
  if (btn) btn.classList.add('active');
  localStorage.setItem('maths4gt_music', type);
  if (type === 'off') return;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const savedTheme   = localStorage.getItem('maths4gt_theme')   || 'default';
  const savedAnim    = localStorage.getItem('maths4gt_anim')    || 'normal';
  const savedBg      = localStorage.getItem('maths4gt_bg')      || 'particles';
  const savedFx      = localStorage.getItem('maths4gt_flashcard_fx') || '';
  const savedSize    = localStorage.getItem('maths4gt_textsize') || 'normal';
  const savedPomoDisplay = localStorage.getItem('maths4gt_pomo_display') || 'float';
