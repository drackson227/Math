...
let allQuestions = [
  ...
  {q: "La condition d'existence d'un cercle est :", opts: ["a² + b² − 4c > 0", "a² + b² − 4c = 0", "a² + b² − 4c &lt; 0", "a + b + c > 0"], ans: 0, chapter: "cercle", difficulty: "intermediaire",
    ...
    simple: "On ne peut pas prendre la racine carrée d'un nombre négatif. Comme le rayon contient \\(\\sqrt{a^2+b^2-4c}\\), il faut que ce nombre soit positif, sinon le cercle n'existe pas."},  {q: "Deux droites \\(y=3x+1\\) et \\(y=-\\dfrac{1}{3}x+5\\) sont :", opts: ["Perpendiculaires", "Parallèles", "Confondues", "Sécantes non perpendiculaires"], ans: 0, chapter: "droite", difficulty: "facile",
...
,
  ...
  {q: "La norme du vecteur \\(\\vec{u}=\\begin{pmatrix}6\\\\8\\end{pmatrix}\\) est :", opts: ["10", "14", "\\(\\sqrt{14}\\)", "48"], ans: 0, chapter: "vecteur", difficulty: "facile",
  ...
  {q: "Les composantes de \\(\\vec{AB}\\) avec A(2 ; −1) et B(5 ; 3) sont :", opts: ["\\(\\begin{pmatrix}3\\\\4\\end{pmatrix}\\)", "\\(\\begin{pmatrix}7\\\\2\\end{pmatrix}\\)", "\\(\\begin{pmatrix}-3\\\\-4\\end{pmatrix}\\)", "\\(\\begin{pmatrix}3\\\\-4\\end{pmatrix}\\)"], ans: 0, chapter: "vecteur", difficulty: "facile",
    exp: "\\(\\vec{AB}=\\begin{pmatrix}x_B-x_A\\\\y_B-y_A\\end{pmatrix}=\\begin{pmatrix}5-2\\\\3-(-1)\\end{pmatrix}=\\begin{pmatrix}3\\\\4\\end{pmatrix}\\)",
    formula: "\\(\\vec{AB}=\\begin{pmatrix}x_B-x_A\\\\y_B-y_A\\end{pmatrix}\\)",
  ...
  {q: "Les vecteurs \\(\\vec{u}=\\begin{pmatrix}2\\\\3\\end{pmatrix}\\) et \\(\\vec{v}=\\begin{pmatrix}4\\\\6\\end{pmatrix}\\) sont :", opts: ["Colinéaires", "Orthogonaux", "Égaux", "De sens opposés"], ans: 0, chapter: "vecteur", difficulty: "intermediaire",
  ...
  {q: "Un vecteur normal à la droite \\(3x-4y+1=0\\) est :", opts: ["\\(\\begin{pmatrix}3\\\\-4\\end{pmatrix}\\)", "\\(\\begin{pmatrix}4\\\\3\\end{pmatrix}\\)", "\\(\\begin{pmatrix}-4\\\\3\\end{pmatrix}\\)", "\\(\\begin{pmatrix}3\\\\4\\end{pmatrix}\\)"], ans: 0, chapter: "droite", difficulty: "intermediaire",
    exp: "Pour une droite \\(ax+by+c=0\\), le vecteur normal est \\(\\vec{n}=\\begin{pmatrix}a\\\\b\\end{pmatrix}=\\begin{pmatrix}3\\\\-4\\end{pmatrix}\\).",
    formula: "\\(\\vec{n}=\\begin{pmatrix}a\\\\b\\end{pmatrix}\\)  pour  \\(ax+by+c=0\\)",
...
let flashcards = [
  ...
  {front: "Ne pas confondre : colinéaires vs orthogonaux ?", back: "• <strong>Colinéaires</strong> (parallèles, même direction) → <strong>déterminant nul</strong> : \\(x\\,y'-y\\,x'=0\\).<br>• <strong>Orthogonaux</strong> (perpendiculaires) → <strong>produit scalaire nul</strong> : \\(x\\,x'+y\\,y'=0\\).<br><em>Astuce : produit « en croix » = colinéaires ; produit « droit » = orthogonaux.</em>", chapter: "vecteur"},
  {front: "Équation vectorielle d'une droite (point A, vecteur directeur \\(\\vec{u}\\)) ?", back: "$$\\vec{AP}=k\\,\\vec{u}=k\\,\\vec{AB}$$ Tout point \\(P\\) de la droite s'obtient en partant de \\(A\\) et en ajoutant \\(k\\) fois le vecteur directeur. Quand \\(k\\) parcourt tous les réels (\\(-\\infty\\) à \\(+\\infty\\)), on décrit <strong>toute</strong> la droite.", chapter: "droite"},
  {front: "Les 4 façons d'écrire une droite ?", back: "• <strong>Vectorielle</strong> : \\(\\vec{AP}=k\\,\\vec{u}\\)<br>• <strong>Paramétriques</strong> : \\(\\begin{cases}x=x_A+k\\,u_x\\\\y=y_A+k\\,u_y\\end{cases}\\)<br>• <strong>Cartésienne (implicite)</strong> : \\(ax+by+c=0\\)<br>• <strong>Explicite</strong> : \\(y=mx+p\\)", chapter: "droite"},
  {front: "Lien entre pente, vecteur directeur et angle d'inclinaison ?", back: "$$m=\\dfrac{u_y}{u_x}=\\dfrac{y_B-y_A}{x_B-x_A}\\qquad \\alpha=\\arctan(m)$$ \\(\\alpha\\) = l'angle que la droite fait avec l'horizontale. Si \\(u_x=0\\) (vecteur directeur vertical), la droite est <strong>verticale</strong> et la pente n'existe pas.", chapter: "droite"},
  {front: "Que représentent m et p dans \\(y=mx+p\\) ?", back: "• <strong>m</strong> = la <strong>pente</strong> : \\(m>0\\) → la droite monte ; \\(m<0\\) → elle descend ; \\(m=0\\) → horizontale.<br>• <strong>p</strong> = l'<strong>ordonnée à l'origine</strong> (là où la droite coupe l'axe \\(y\\)).<br>• Vecteur directeur : \\(\\vec{u}=\\begin{pmatrix}1\\\\m\\end{pmatrix}\\).", chapter: "droite"},
  {front: "Qu'est-ce qu'un cercle (définition) ?", back: "L'<strong>ensemble des points situés à une même distance R</strong> (le rayon) d'un point fixe \\(C\\) (le centre). En équation : \\((x-x_0)^2+(y-y_0)^2=R^2\\).", chapter: "cercle"},
  {front: "Quand une droite est-elle tangente à un cercle ?", back: "Quand la <strong>distance du centre \\(C\\) à la droite</strong> vaut exactement le rayon : $$d(C,\\,d)=R$$ • \\(d(C,d)<R\\) → sécante (2 points)<br>• \\(d(C,d)=R\\) → <strong>tangente</strong> (1 seul point)<br>• \\(d(C,d)>R\\) → aucun point commun.", chapter: "cercle"},
  {front: "Opérations sur les composantes de \\(\\vec{u}=\\begin{pmatrix}x\\\\y\\end{pmatrix}\\) et \\(\\vec{v}=\\begin{pmatrix}x'\\\\y'\\end{pmatrix}\\) ?", back: "• <strong>Somme</strong> : \\(\\vec{u}+\\vec{v}=\\begin{pmatrix}x+x'\\\\y+y'\\end{pmatrix}\\)<br>• <strong>Différence</strong> : \\(\\vec{u}-\\vec{v}=\\begin{pmatrix}x-x'\\\\y-y'\\end{pmatrix}\\)<br>• <strong>Multiple</strong> : \\(k\\,\\vec{u}=\\begin{pmatrix}k\\,x\\\\k\\,y\\end{pmatrix}\\)<br>On calcule composante par composante.", chapter: "vecteur"}
];
...
const focusContent = {
  ...
  droite: [
    ...
    {title: "Forme implicite", content: "$$ax + by + c = 0$$", note: "$\\vec{n}=\\begin{pmatrix}a\\\\b\\end{pmatrix}$, $\\vec{u}=\\begin{pmatrix}-b\\\\a\\end{pmatrix}$"},
    ...
    {title: "Distance point-droite", content: "$$d(P,d) = \\frac{|ax_P+by_P+c|}{\\sqrt{a^2+b^2}}$$", note: "Distance la plus courte — la valeur absolue est indispensable !"},
